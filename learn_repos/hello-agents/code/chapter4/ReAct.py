...
if __name__ == '__main__':
    ...
    llm = HelloAgentsLLM()
    tool_executor = ToolExecutor()
    search_desc = "一个安全的数学计算器。适合处理算术、函数、三角函数、对数、平方根、阶乘、最大最小值等复杂计算。"
    tool_executor.registerTool("Calculator", search_desc, calculator)
    agent = ReActAgent(llm_client=llm, tool_executor=tool_executor)
    question = "一个投资者有 12000 元本金，第一年收益率为 8%，第二年收益率为 -3%，第三年收益率为 12%。三年后本金变成多少？总收益率是多少？"
    agent.run(question)
