...
class ThreeKingdomsWerewolfGame:
    ...
    async def create_player(self, role: str, character: str) -> ReActAgent:
        ...
        agent = ReActAgent(
            ...
            model=OpenAIChatModel(
                model_name=os.getenv("LLM_MODEL_ID", "gpt-4o-mini"),
    ...
    async def hunter_phase(self, shot_by_hunter: str, available_players: Optional[List[ReActAgent]] = None):
        """猎人阶段"""
        hunter_role_alive = [player for player in self.hunter if player.name == shot_by_hunter]
        if not hunter_role_alive:
            return None
            
        hunter_agent = hunter_role_alive[0]
        if hunter_agent.name == shot_by_hunter:
            await self.moderator.announce("🏹 猎人发动技能，可以带走一名玩家...")

            candidates = [
                player for player in (available_players or self.alive_players)
                if player.name != hunter_agent.name
            ]
            if not candidates:
                print(f"⚠️ 没有可供猎人{hunter_agent.name}开枪的有效目标")
                return None
            
            hunter_action = await hunter_agent(
                structured_model=get_hunter_model_cn(candidates)
            )
    ...
    async def run_game(self):
        ...
        try:
            ...
            for round_num in range(1, MAX_GAME_ROUND + 1):
                ...
                alive_before_night_deaths = list(self.alive_players)
                ...
                # 猎人若被狼人夜晚击杀，技能在白天天亮公布死亡后发动；
                # 被女巫毒杀不触发猎人技能。
                night_hunter_shot = await self.hunter_phase(
                    final_killed,
                    available_players=[
                        player for player in alive_before_night_deaths
                        if player.name not in night_deaths
                    ],
                )
                if night_hunter_shot:
                    self.update_alive_players([night_hunter_shot])
